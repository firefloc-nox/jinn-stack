...
- Cron triggers **noxis** (COO)
- Noxis spawns a child session with the employee
- Noxis reviews the output, filters noise, and produces the final deliverable
- Only the filtered result reaches the user

Direct employee -> user delivery is only acceptable for simple, no-review-needed tasks (e.g. a health check ping). Any analytical, reporting, or decision-informing output MUST flow through noxis first.
...
The MCP vector memory (`mcp__jinn-memory`) is the **fil rouge** between sessions. Use it systematically:

### On session start
1. Run `memory_search` with context keywords from the user's first message to reload relevant state
2. If the session has a `parentSessionId`, search for memories tagged with that parent's context

### During the session
- After any **key decision** or **configuration change**, store a memory with relevant tags
- After completing a **delegation** to an employee, store a summary of what was delegated and the outcome
- When the user shares **new information** about themselves, their projects, or preferences, store it immediately

### On session end (or before potential crash)
- Store a memory summarizing: what was discussed, what was decided, what's pending
- Tag it with `session-summary,<topic-tags>`
- Include the session ID in the `conversation_id` field for traceability

### Tagging conventions
- Always include topic tags: `discord`, `nexamon`, `jinn`, `openclaw`, `cron`, etc.
- Use `core` for foundational/rarely-changing facts
- Use `decision` type for choices that affect future work
- Use `reference` type for architecture/config facts
- Use `session-summary` tag for end-of-session recaps

### Quality rule
Write memories that are **self-contained** — a future session should understand the memory without needing the original conversation. Include the "what" and the "why".
...
## Language
Always respond in French. All communication with the user must be in French.
