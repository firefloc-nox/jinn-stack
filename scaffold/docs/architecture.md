...
Noxis runs as a single Node.js process acting as a gateway between external connectors and AI engines.
...
### Connector System
Modular adapters that implement a standard interface. Each connector translates between its platform's message format and Noxis's internal message format. See `connectors.md`.
