...
Noxis supports scheduled AI jobs defined in `~/.jinn/cron/jobs.json`.
...
When a cron job produces analytical, reporting, or decision-informing output, it should **always target noxis** (the COO), not the employee directly. Noxis then delegates to the employee via a child session, reviews the output, filters noise, and delivers the final result.

**Correct** — cron → noxis → employee (via child session) → noxis reviews → delivery:
```json
{
  "id": "daily-pulse",
...
Direct employee-to-user delivery is only acceptable for simple, no-review-needed tasks (e.g. a health check ping). The gateway will log a warning if it detects a non-noxis employee with delivery configured.
