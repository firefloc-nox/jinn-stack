...
Noxis automatically configures MCP servers for AI engine sessions, giving employees access to browser automation, web search, and gateway tools without manual setup.

## How It Works

1. MCP servers are defined in `config.yaml` under the `mcp:` section
2. When a session starts, Noxis resolves which MCP servers the employee needs
...
### Gateway
Built-in MCP server that wraps Noxis's own API. Gives employees tools to:
