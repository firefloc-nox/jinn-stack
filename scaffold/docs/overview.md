...
Noxis is a lightweight AI gateway daemon that wraps professional AI CLI tools as "engines." It is published as `jinn-cli`.

## Core Principle

**Noxis is a bus, not a brain.** All AI intelligence comes from the engines natively. Noxis adds no custom agentic loop, no prompt engineering layer, no opinions on how AI should behave. It delegates everything to professional tools (Claude Code CLI, Codex SDK) and focuses solely on routing, scheduling, and connectivity.
...
- **Self-modification**: Noxis can edit its own config, skills, cron jobs, and org structure at runtime

## How It Differs from Custom Agentic Frameworks

Traditional approaches build custom tool-calling loops, manage context windows, and implement retry logic. Noxis does none of that. The engines (Claude Code, Codex) already handle tool use, file editing, command execution, and multi-step reasoning. Noxis just connects them to the outside world.
