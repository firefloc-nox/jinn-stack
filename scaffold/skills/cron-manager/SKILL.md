...
6. **Delegation check**: If the job has delivery configured AND targets a non-noxis employee, warn the user. The correct pattern for reporting/analytical jobs is: target `noxis`, and include delegation instructions in the prompt (e.g. "Delegate to @employee-name: ..."). Noxis reviews and filters the output before it reaches the delivery channel. Only simple, no-review tasks (e.g. health checks) should target employees directly with delivery.
