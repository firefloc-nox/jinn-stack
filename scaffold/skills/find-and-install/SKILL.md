...
### Step 2: Copy into noxis skills directory

```bash
cp -r ~/.claude/skills/<name>/ ~/.noxis/skills/<name>/
```

The Noxis file watcher will detect the new directory and create the appropriate symlinks automatically.

### Step 3: Update the skills manifest

Read `~/.noxis/skills.json`, add the new skill entry, and write it back.
...
Read the newly installed `~/.noxis/skills/<name>/SKILL.md` and follow its instructions to complete the current task.
...
cp -r ~/.claude/skills/vercel-deploy/ ~/.noxis/skills/vercel-deploy/
