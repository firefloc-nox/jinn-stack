...
The organization lives under the `org/` directory in the Noxis home folder (`~/.jinn/org/`). Each department is a subdirectory containing employee persona YAML files and a task board.
...
persona: |
  ...
  - Report summaries back to the COO (Noxis), not raw employee output
