---
name: self-heal
description: Diagnose and fix problems in Noxis's configuration and runtime
---
...
This skill activates when Noxis is experiencing issues, errors, or unexpected behavior. It also activates when the user asks to diagnose problems, check system health, or fix something that is not working.
...
2. If another Noxis instance is running, tell the user to stop it first.
3. If a different process is using the port, suggest changing the port in `config.yaml`.

## Reference

For understanding Noxis's architecture and component relationships, refer to the documentation in `~/.jinn/docs/` if available.
